...
function getAverage(value1, value2) {
  let result = 0;
  if (value1 / 2 + value2 / 2 <= Number.MAX_VALUE) {
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const getCoordinatesX = (x2 - x1) ** 2;
  const getCoordinatesY = (y2 - y1) ** 2;
  const result = Math.sqrt(getCoordinatesX + getCoordinatesY);
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosinus =
    vec1 + (vec2 / Math.sqrt(x1 ** 2 + y1 ** 2)) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const arccos = Math.acos(cosinus);
  return arccos;
}
...
function roundToPowerOfTen(num, pow) {
  const digitPow = `1${'0'.repeat(pow)}`;
  return Math.round(num / digitPow) * digitPow;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i < index; i += 1) {
    currentFibonachi.push(currentFibonachi[i - 2] + currentFibonachi[i - 1]);
  }
  return currentFibonachi[index - 1];
}
